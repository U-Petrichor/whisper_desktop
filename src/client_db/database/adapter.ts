...
class DatabaseAdapter {
  ...
  async status() {
    if (this.isElectron) {
      ...
      try {
        const dexieDb = this.db as any;
        const messageCount = await dexieDb.messages.count();
        const contactCount = await dexieDb.contacts.count();
        const conversationCount = await dexieDb.conversations.count();
        const userKeys = await dexieDb.userKeys.get(this.userId);
